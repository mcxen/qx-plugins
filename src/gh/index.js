...
const CACHE_KEY = "gh.html.bundle.v2";
const DEFAULT_REPOS = "mcxen/qx\nmcxen/qx-plugins";
const UA = "Qx-Plugin-gh/1.2 (+https://github.com/mcxen/qx; public page reader)";
  ...
  commands: [
    {
      ...
      async run(context) {
        context.showToast("Open the GitHub panel (reads public github.com pages)");
      },
    },
    {
      name: "refresh-gh",
    ...
    {
      name: "gh-watch-island",
      title: "Watch CI on Island",
      async run(context) {
        try {
          const b = await loadBundle(context, { force: true });
          const hot = pickHottestRun(b.runs);
