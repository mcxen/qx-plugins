...
// raycast-source/bing-wallpaper/src/set-bing-wallpaper.tsx
var set_bing_wallpaper_exports = {};
__export(set_bing_wallpaper_exports, {
  default: () => CommonDirectory
});
...
async function showToast(input, title, message) {
  ...
  return {
    ...
    show() {
      runtime()?.context?.showToast?.(this.title || "");
    }
  };
}
...
function ItemShell({ title, subtitle, icon, accessories, actions, children, image }) {
  ...
  return import_react.default.createElement(
    ...
    import_react.default.createElement(
      ...
      import_react.default.createElement("strong", null, textOf(title)),
...
List.Section = function ListSection({ title, children }) {
  return import_react.default.createElement(
    "section",
    { className: "qx-raycast-section" },
    title ? import_react.default.createElement("h2", null, title) : null,
    children
  );
};
...
var { layout, columns, applyTo, downloadSize, downloadDirectory, autoDownload, includeDownloadedWallpapers } = getPreferenceValues();
var { refreshInterval } = getPreferenceValues();

// qx-virtual:node-fetch
...
var buildBingWallpapersURL = (wallpaperDate = 0, count = 8) => {
...
var buildBingImageURL = (urlSuffix, downloadSize2 = "raw", high = 3840, width = 2160) => {
  let _urlSuffix;
  switch (downloadSize2) {
    case "raw": {
...
var buildCopyrightURL = (urlSuffix) => {
  return "https://www.bing.com" + urlSuffix;
};
var getPictureName = (url) => {
...
function writes() {
  if (!globalThis.__qxRaycastPendingWrites) globalThis.__qxRaycastPendingWrites = /* @__PURE__ */ new Map();
...
async function pathExists(path) {
  ...
  try {
    const exists = await runtime2()?.context?.qx?.invokeRust?.("plugin_file_exists", { path: value });
...
async function emptydir(dir) {
  ...
  dirs.add(prefix);
  await runtime2()?.context?.qx?.invokeRust?.("plugin_file_empty_dir", { path: prefix });
}
async function ensureDir(dir) {
  const target = normalizePath(dir);
  rememberDir(target);
  await runtime2()?.context?.qx?.invokeRust?.("plugin_file_ensure_dir", { path: target });
}
var mkdirp = ensureDir;
function writeFile(path, data, callback) {
  const target = normalizePath(path);
  rememberDir(dirname(target));
  mem.set(target, data);
  const promise = runtime2()?.context?.qx?.invokeRust?.("plugin_file_write_base64", {
    path: target,
...
async function readFile(path, options, callback) {
  ...
  const target = normalizePath(path);
  const promise = mem.has(target) ? Promise.resolve(toUint8Array(mem.get(target))) : runtime2()?.context?.qx?.invokeRust?.("plugin_file_read_base64", { path: target }).then(fromBase64);
...
var fs_extra_default = { existsSync, pathExistsSync, pathExists, readdirSync, emptydir, ensureDir, mkdirp, writeFile, writeFileSync, readFile, readFileSync, readJson, writeJson };

// qx-virtual:run-applescript
async function runAppleScript(script) {
  const pending = globalThis.__qxRaycastPendingWrites ? [...globalThis.__qxRaycastPendingWrites.values()] : [];
  if (pending.length) await Promise.allSettled(pending);
  return globalThis.__qxRaycastRuntime?.context?.qx?.invokeRust?.("plugin_run_applescript", { script: String(script || "") }).catch(() => "unsupported");
}

// qx-virtual:os
function homedir() {
  return globalThis.__qxRaycastRuntime?.homeDirectory || "/qx-home";
}

// raycast-source/bing-wallpaper/src/utils/constants.ts
var WallpaperTag = /* @__PURE__ */ ((WallpaperTag2) => {
  WallpaperTag2["ALL"] = "All";
  WallpaperTag2["ONLINE"] = "Online";
  WallpaperTag2["DOWNLOADED"] = "Downloaded";
  return WallpaperTag2;
})(WallpaperTag || {});
var wallpaperTags = Object.entries(WallpaperTag);
var wallpaperImageExtension = [".png", ".jpg", ".heic"];
...
// raycast-source/bing-wallpaper/src/utils/common-utils.ts
var cachePath = environment.supportPath;
var isEmpty = (string) => {
  return !(string != null && String(string).length > 0);
};
var buildCachePath = (title) => {
  const normalizedCachePath = cachePath.endsWith("/") ? cachePath : `${cachePath}/`;
  return `${normalizedCachePath}${title}.png`;
};
async function cachePicture(title, url) {
  ...
  if (res) {
    ...
    fs_extra_default.writeFile(buildCachePath(title), Buffer.from(buffer), async (error) => {
      if (error) {
...
var setOnlineWallpaper = async (title, url, isShowToast = true) => {
  const toast = isShowToast ? await showToast(Toast.Style.Animated, "Downloading wallpaper...") : void 0;
  await fs_extra_default.emptydir(cachePath);
  try {
...
var setLocalWallpaper = async (path, isShowToast = true) => {
  const toast = isShowToast ? await showToast(Toast.Style.Animated, "Setting wallpaper...") : void 0;
  try {
...
async function autoDownloadPictures(downSize, bingImages) {
  bingImages.forEach((value) => {
    const picturePath = `${getPicturesDirectory()}/${getPictureName(value.url)}-${value.startdate}.png`;
    if (!fs_extra_default.existsSync(picturePath)) {
      fetch(buildBingImageURL(value.url, downSize)).then(function(res) {
        return res.arrayBuffer();
      }).then(function(buffer) {
        fs_extra_default.writeFile(picturePath, Buffer.from(buffer), async (error) => {
          if (error) {
...
function getDownloadedBingWallpapers() {
  const downloadedWallpapers = [];
  try {
    const imageFolderPath = getPicturesDirectory();
    const files = fs_extra_default.readdirSync(imageFolderPath);
    files.forEach((value) => {
...
// raycast-source/bing-wallpaper/src/hooks/hooks.ts
var getBingWallpapers = (showDownloadedWallpapers) => {
  const [bingWallpaperHD, setBingWallpaperHD] = (0, import_react2.useState)([]);
  const [downloadedBingWallpapers, setDownloadedBingWallpapers] = (0, import_react2.useState)([]);
  const [isLoading, setIsLoading] = (0, import_react2.useState)(true);
  const fetchData = (0, import_react2.useCallback)(async () => {
    try {
      fetch(buildBingWallpapersURL(0, 8)).then((first_res) => first_res.json()).then((first_data) => {
        fetch(buildBingWallpapersURL(8, 8)).then((second_res) => second_res.json()).then((second_data) => {
          second_data.images.shift();
          const _bingWallpaperHD = first_data.images.concat(
            second_data.images
          );
  ...
  return { bingWallpaperHD, downloadedBingWallpapers, isLoading };
};
var autoDownloadWallpapers = (bingWallpapers) => {
  const fetchData = (0, import_react2.useCallback)(async () => {
    try {
      if (autoDownload) {
...
// raycast-source/bing-wallpaper/src/components/wallpaper-list-layout.tsx
var import_react4 = __toESM(require_react());

// raycast-source/bing-wallpaper/src/components/action-open-extension-preferences.tsx
var import_jsx_runtime = __toESM(require_jsx_runtime());
function ActionOpenExtensionPreferences() {
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ActionPanel.Section, { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
    Action,
    {
      icon: Icon.Gear,
...
// raycast-source/bing-wallpaper/src/components/list-empty-view.tsx
var import_jsx_runtime2 = __toESM(require_jsx_runtime());
function ListEmptyView2(props) {
  const { layout: layout2 } = props;
  return layout2 === "List" ? /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(
    List.EmptyView,
    {
      icon: { source: { light: "empty-view-icon.png", dark: "empty-view-icon@dark.png" } },
      title: "No wallpapers",
      description: "",
      actions: /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(ActionPanel, { children: /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(ActionOpenExtensionPreferences, {}) })
    }
...
// raycast-source/bing-wallpaper/src/preview-bing-wallpaper.tsx
...
function PreviewBingWallpaper(props) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime3.jsx)(
    Detail,
    {
      navigationTitle: pageIndex.isOnline ? getPictureName(onlineImages[pageIndex.index].url) : downloadedImage[pageIndex.index - onlineImages.length].name,
      markdown: `<img src="${pageIndex.isOnline ? buildBingImageURL(onlineImages[pageIndex.index].url, "icon", 960, 540) : fileUrl(downloadedImage[pageIndex.index - onlineImages.length].path)}" alt="" height="360" />`,
      actions: /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)(ActionPanel, { children: [
        ...
        /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)(ActionPanel.Section, { children: [
          /* @__PURE__ */ (0, import_jsx_runtime3.jsx)(
            Action,
            {
              icon: Icon.Desktop,
          ...
          pageIndex.isOnline && /* @__PURE__ */ (0, import_jsx_runtime3.jsx)(
            Action,
            {
              icon: Icon.Download,
        ...
        /* @__PURE__ */ (0, import_jsx_runtime3.jsx)(ActionOpenExtensionPreferences, {})
      ] })
    }
  );
}

// raycast-source/bing-wallpaper/src/components/actions-online-bing-wallpaper.tsx
var import_jsx_runtime4 = __toESM(require_jsx_runtime());
function ActionsOnlineBingWallpaper(props) {
  const { index, bingImage, onlineImages, downloadedImages } = props;
  return /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)(ActionPanel, { children: [
    ...
    /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(
      Action.Push,
      {
        ...
        shortcut: { modifiers: ["cmd"], key: "y" },
        target: /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(
    ...
    /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)(ActionPanel.Section, { children: [
      /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(
        Action,
        {
          icon: Icon.Repeat,
    ...
    /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(ActionOpenExtensionPreferences, {})
  ] });
}

// raycast-source/bing-wallpaper/src/components/actions-downloaded-bing-wallpaper.tsx
var import_jsx_runtime5 = __toESM(require_jsx_runtime());
function ActionsDownloadedBingWallpaper(props) {
  const { index, bingImage, onlineImages, downloadedImages } = props;
  return /* @__PURE__ */ (0, import_jsx_runtime5.jsxs)(ActionPanel, { children: [
    ...
    /* @__PURE__ */ (0, import_jsx_runtime5.jsx)(Action.ShowInFinder, { path: bingImage.path }),
    /* @__PURE__ */ (0, import_jsx_runtime5.jsx)(
      Action.Push,
      {
        icon: Icon.Sidebar,
        title: "Preview Wallpaper",
        shortcut: { modifiers: ["cmd"], key: "y" },
        target: /* @__PURE__ */ (0, import_jsx_runtime5.jsx)(
          PreviewBingWallpaper,
          {
            isOnline: false,
            index,
    ...
    /* @__PURE__ */ (0, import_jsx_runtime5.jsxs)(ActionPanel.Section, { children: [
      /* @__PURE__ */ (0, import_jsx_runtime5.jsx)(
        Action,
        {
    ...
    /* @__PURE__ */ (0, import_jsx_runtime5.jsx)(ActionOpenExtensionPreferences, {})
  ] });
}

// raycast-source/bing-wallpaper/src/components/wallpaper-list-layout.tsx
var import_jsx_runtime6 = __toESM(require_jsx_runtime());
function WallpaperListLayout(props) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime6.jsxs)(
    List,
    {
      ...
      searchBarAccessory: includeDownloadedWallpapers ? /* @__PURE__ */ (0, import_jsx_runtime6.jsx)(List.Dropdown, { onChange: setTag, tooltip: "Wallpaper type", storeValue: true, children: wallpaperTags.map((value, index) => {
        return /* @__PURE__ */ (0, import_jsx_runtime6.jsx)(List.Dropdown.Item, { title: value[1], value: value[1] }, index + "_" + value[0]);
      }) }) : null,
      children: [
        /* @__PURE__ */ (0, import_jsx_runtime6.jsx)(ListEmptyView2, { layout }),
        (tag === "All" /* ALL */ || tag === "Online" /* ONLINE */ || tag === "") && /* @__PURE__ */ (0, import_jsx_runtime6.jsx)(List.Section, { title: "Online Wallpapers", children: bingWallpaperHD.map((bingImage, index) => {
          return /* @__PURE__ */ (0, import_jsx_runtime6.jsx)(
            List.Item,
            {
              ...
              detail: /* @__PURE__ */ (0, import_jsx_runtime6.jsx)(
                List.Item.Detail,
                {
                  isLoading: false,
...
${bingImage.copyright}`
            ...
            index + bingImage.startdate
          );
        }) }),
        includeDownloadedWallpapers && (tag === "All" /* ALL */ || tag === "Downloaded" /* DOWNLOADED */) && /* @__PURE__ */ (0, import_jsx_runtime6.jsx)(List.Section, { title: "Downloaded Wallpapers", children: downloadedBingWallpapers?.map((bingImage, index) => {
          return /* @__PURE__ */ (0, import_jsx_runtime6.jsx)(
            List.Item,
            {
...
// raycast-source/bing-wallpaper/src/components/wallpaper-grid-layout.tsx
var import_react5 = __toESM(require_react());
var import_jsx_runtime7 = __toESM(require_jsx_runtime());
function WallpaperGridLayout(props) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime7.jsxs)(
    Grid,
    {
      ...
      fit: Grid.Fit.Fill,
      searchBarPlaceholder: "Search wallpapers",
      searchBarAccessory: includeDownloadedWallpapers ? /* @__PURE__ */ (0, import_jsx_runtime7.jsx)(Grid.Dropdown, { onChange: setTag, tooltip: "Wallpaper type", storeValue: true, children: wallpaperTags.map((value, index) => {
        return /* @__PURE__ */ (0, import_jsx_runtime7.jsx)(Grid.Dropdown.Item, { title: value[1], value: value[1] }, index + "_" + value[0]);
      }) }) : null,
      children: [
        /* @__PURE__ */ (0, import_jsx_runtime7.jsx)(ListEmptyView2, { layout }),
        (tag === "All" /* ALL */ || tag === "Online" /* ONLINE */ || tag === "") && /* @__PURE__ */ (0, import_jsx_runtime7.jsx)(Grid.Section, { title: "Online Wallpapers", children: bingWallpaperHD.map((bingImage, index) => {
        ...
        includeDownloadedWallpapers && (tag === "All" /* ALL */ || tag === "Downloaded" /* DOWNLOADED */) && /* @__PURE__ */ (0, import_jsx_runtime7.jsx)(Grid.Section, { title: "Downloaded Wallpapers", children: downloadedBingWallpapers?.map((bingImage, index) => {
          return /* @__PURE__ */ (0, import_jsx_runtime7.jsx)(
            Grid.Item,
            {
              id: index + bingImage.name,
...
function CommonDirectory() {
  ...
  return layout === "List" ? /* @__PURE__ */ (0, import_jsx_runtime8.jsx)(
    WallpaperListLayout,
    {
      isLoading,
      bingWallpaperHD,
      downloadedBingWallpapers
    }
...
// raycast-source/bing-wallpaper/src/auto-random-bing-wallpaper.ts
var auto_random_bing_wallpaper_exports = {};
__export(auto_random_bing_wallpaper_exports, {
  default: () => auto_random_bing_wallpaper_default,
  getRandomWallpaper: () => getRandomWallpaper
});

// raycast-source/bing-wallpaper/src/utils/refresh-record.ts
var cache = new Cache();
var ONE_MINUTE = 60 * 1e3;
var KEY_LAST_REFRESH = "lastRefresh";
function canRefresh() {
...
// raycast-source/bing-wallpaper/src/auto-random-bing-wallpaper.ts
var auto_random_bing_wallpaper_default = async () => {
...
var getRandomWallpaper = async () => {
  ...
  try {
    const firstResponseDataImages = (await firstResponse.json()).images;
    const secondResponseDataImages = (await secondResponse.json()).images;
...
// raycast-source/bing-wallpaper/src/auto-switch-bing-wallpaper.ts
var auto_switch_bing_wallpaper_exports = {};
__export(auto_switch_bing_wallpaper_exports, {
  default: () => auto_switch_bing_wallpaper_default
});
var auto_switch_bing_wallpaper_default = async () => {
  if (environment.launchType === LaunchType.UserInitiated) {
    await showHUD("Downloading and setting wallpaper...");
  }
  await getLatestWallpaper();
};
var getLatestWallpaper = async () => {
  const firstResponse = await fetch(buildBingWallpapersURL(0, 8)).catch(async (e) => {
    console.error(e);
    return void 0;
  });
  ...
  try {
    const bingWallpaperHD = (await firstResponse.json()).images;
    if (autoDownload) {
      await autoDownloadPictures(downloadSize, bingWallpaperHD);
    }
...
var commandModules = {
  "set-bing-wallpaper": set_bing_wallpaper_exports,
  "auto-random-bing-wallpaper": auto_random_bing_wallpaper_exports,
  "auto-switch-bing-wallpaper": auto_switch_bing_wallpaper_exports
};
...
async function invokeCommand(name, container, context) {
  ...
  if (import_react6.default.isValidElement(result)) {
    ...
    renderElement(container, import_react6.default.createElement("div", { className: "qx-raycast-empty" }, result == null ? "" : String(result)));
  }
}
var entry_default = {
