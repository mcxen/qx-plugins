...
const CACHE_KEY = "qxheihe.feed.v1";
...
async function cacheSet(context, data) {
  try {
    await context.storage.persist.set(CACHE_KEY, { data, savedAt: Date.now() });
  } catch {
...
function createPanel(container, context) {
  ...
  async function loadFeed({ force = false } = {}) {
    ...
    try {
      ...
      if (!force && Array.isArray(cached?.data) && cacheAge <= MAX_STALE_MS) {
  ...
  async function loadMore() {
    ...
    try {
      ...
      for (const post of incoming) byId.set(String(post.linkid), post);
      ...
      await cacheSet(context, state.all);
    } catch (error) {
