...
async function safeImagePreview(bytes, type, kind) {
  ...
  try {
    ...
    let scale = Math.min(1, maxEdge / Math.max(width, height), Math.sqrt(maxPixels / (width * height)));
...
function createPanel(container, context) {
  ...
  function commentsSections(post) {
    const comments = state.cache.details[post.id]?.comments || [];
    if (!comments.length) return [];
    return comments.map((comment, index) => ({
      title: `${comment.user?.screenName || copy("Unknown user", "未知用户")} · #${index + 1} · ${formatDate(comment.createdAt)}`,
      body: [comment.text, comment.replyText ? `${copy("Reply", "回复")}：${comment.replyText}` : ""]
        .filter(Boolean).join("\n"),
      fields: comment.likesCount ? [{
        label: copy("Likes", "点赞"),
        value: formatCount(comment.likesCount),
      }] : undefined,
    }));
  }
  ...
  function detailFor(post) {
    ...
    return {
      ...
      sections: commentsSections(post),
    };
  }
